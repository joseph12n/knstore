...
- **2026-09-23 (pendiente de commit, Fase 2c):** `scripts/seed-operaciones.js` (seed idempotente de operaciones vía API: clientes, cuentas, direcciones, checkout, estados de pedido/envío y resumen) + CSP con `https://images.pexels.com` en `application.yml`. Doble corrida sin duplicados verificada contra un mock de la API (no había backend en `localhost:8080`); **falta re-ejecutarlo contra el backend real**. Detalle en `BITACORA.md`.
