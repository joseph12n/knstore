...
export const useCart = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated || !account.login || initialized) {
      return;
    }
    ...
    const init = async () => {
      setLoading(true);
      try {
        const [cuenta, productos] = await Promise.all([findCuenta(account.login), fetchProductos()]);
  ...
  }, [isAuthenticated, account.login, initialized]);
  ...
  const addItem = useCallback(
    async (producto: IProductoStorefront, cantidad = 1) => {
      ...
      if (existing?.id) {
        ...
        try {
          const cuenta = await findCuenta(account.login);
          if (!cuenta?.id) return;
          const carrito = await findOrCreateCarrito(cuenta.id);
          carritoIdRef.current = carrito.id;
          const response = await axios.post<IItemCarrito>('api/item-carritos', {
    ...
    [isAuthenticated, serverItems, account.login],
  );
