...
> Resume el hito **2026-09-23/24**: release **3.3.0** (contenido v3 con 60 productos e imágenes locales de Zappos, fix de duplicación del Quality Gate de SonarCloud, imagen en Docker Hub y despliegue en la EC2) y el **wipe + re-siembra completa de la BD de producción del 2026-09-24** (60 productos/179 imágenes + operaciones demo). Además, el **2026-09-24** se reescribió la historia de Git (§1): `main` en 25 hitos y ramas con la obra de cada autor. Incluye el handoff previo del 2026-09-18 (calidad H-01→H-08, EC2) y el flujo de contenido del 2026-09-22.

---

## 1. Git — estado

- **2026-09-24 — historia reescrita (aprobada por el responsable):** `main` pasó de **176 commits lineales a 25 hitos** con resúmenes de cada integración; el árbol es idéntico al anterior (**contenido sin cambios**). Las **5 ramas** dejaron de apuntar a `main`: `joseph` (historia cruda completa, 176 commits, identidad unificada a `joseph12n`; cadena `27877d3`), `carrito` (base + 10 commits de su línea; cadena `c7416bf`), `Nicolas` (20 commits + 7 snapshots; cadena `2d5a044`), `lauraG` (8 + 3; cadena `5f60c9f`) y `santiago` (16 + 6; cadena `c239635`). Cada rama lleva al frente un `Merge branch 'main'` y su contenido final es igual al de `main`; los snapshots van a nombre de `KN-Store <equipo@knstore.local>`.
- **Convención nueva (`AGENTS.md` §12.1):** las ramas **no** se vuelven a unificar apuntándolas a `main`; conservan la obra de su autor (commits completos, sin unificar) y se sincronizan con `git merge main`.
- **Tags remapeados a los hitos equivalentes:** `v3.0.0` → `2902bd5`, `v3.0.1` → `4c86424`, `v3.0.3` → `6cefab5`, `v3.1.0` → `2e500ed`, `v3.1.1` → `5e3e725`, `v3.2.0` → `4abc2de`, `v3.3.0` → `86f94e5` (objetos anotados `865fd30`, `185603a`, `e563fa6`, `aaa6b31`, `b022bc0`, `390df5a` y `edad66e`); último hito de `main` = `29b3e96` + commit de registro encima. Los hitos llevan **autoría distribuida**: `7dd5c4d` (endpoints/smtp) por Santiago, `5fe6f2d` (cuenta/direcciones/pagos) por Nicolás y `de68907` (manuales) por Laura; el resto por el responsable, con `Co-authored-by` de los aportes (todos figuran en `main`).
- **Respaldo de la historia anterior:** `~/Documentos/knstore-pre-reescritura-2026-09-24.bundle` (74 MB, 77 refs; `git bundle verify` OK) + `refs/backup/2026-09-24-pre-reescritura/*` (6 ramas + 7 tags). Los SHAs previos citados en esta documentación (`438293f`, `2ef03c9`, `77e5403`, `b26be02`, …) solo se resuelven desde el bundle/refs de respaldo.
- **Publicación (2026-09-24, `auditor`):** en `origin` (joseph12n/knstore) quedaron `main` `8d74df1` + 5 ramas + 7 tags (verificado por `ls-remote`); en **sena-students** se publicaron las 5 ramas y los 7 tags, pero **`main` sigue en `2ef03c9`**: el ruleset de la organización `classroom50-protect-submission-history` rechaza el force push (`GH013`, sin bypass). Para completarlo, un owner debe agregar un bypass/pausar el ruleset y ejecutar `git push --force-with-lease origin main` desde el espejo (su local ya está en la historia nueva).
- **Release 3.3.0 (historia previa):** 7 commits `73e089f`…`77e5403`; fases 2c/3b/4b publicadas en el release 3.2.0 (tag sobre `15f4588`). Imagen publicada en Docker Hub: **`eljoseph12/knstore:3.3.0`** (= `latest`, digest `sha256:2d3b62f1cc94c4e90e0388cd257230aebe5c7240ad5394fed5fda44df0d03a75`; verificada por API el 2026-09-24 04:44 UTC).
- **SonarCloud** (`joseph12n_knstore`): Quality Gate **OK (5/5)** con **1,1 % de duplicación en código nuevo** (umbral 3 %) — análisis `b24e740b…` (rev `77e5403`), `ac037261…` (rev `b26be02`) y `b5eab6b0…` (rev `438293f`), previos a la reescritura (esas revisiones viven en el bundle). Auto-Scan ignora `sonar-project.properties` (ver §5).
